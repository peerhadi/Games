...
type OverworldMapConfig = {
    ...
    cutsceneSpaces?: Record<string, Array<Record<string, any>>>,
    id: string;
}
export class OverworldMap {
    ...
    drawLowerImage(ctx: CanvasRenderingContext2D, cameraPerson: Person) {
        ctx.drawImage(this.lowerImage, utils.withGrid(10.5) - cameraPerson.x, utils.withGrid(6) - cameraPerson.y)
    }

    drawUpperImage(ctx: CanvasRenderingContext2D, cameraPerson: Person) {
        ctx.drawImage(this.upperImage, utils.withGrid(10.5) - cameraPerson.x, utils.withGrid(6) - cameraPerson.y)
    }

    isSpaceTaken(currentX: number, currentY: number, direciton: State) {
        const { x, y } = utils.nextPosition(currentX, currentY, direciton);
        if (this.walls[`${x},${y}`]) {
            return true;
        }
    ...
    mountObjects() {
        Object.keys(this.configObjects).forEach((key) => {
            let o = this.configObjects[key];
            o.id = key;

            let instance;

            if (o.type === 'Person') {
                instance = new Person(o)
            }
            if (o.type === 'PizzaStone') {
                instance = new PizzaStone(o)
            }
    ...
    async startCutscene(events: Array<Behavior>) {
    ...
    checkForActionPause() {
        ...
        if (!this.cutscenePlaying && match.talking.length) {
            const relaventScenario = match.talking.find((scenario: Record<string, any>) => {
                return (scenario.required || []).every((sf: string) => {
                    return playerState.storyFlags[sf]
                })
            })
            relaventScenario && this.startCutscene(relaventScenario.events)
        }
    }
    ...
    Street: {
        ...
        walls: function() {
            ...
            ["4,9", "5,8", "6,9", "7,9", "8,9", "9,9", "10,9", "11,9", "12,9", "13,8", "14,8", "15,7",
                ...
                let [x, y] = coord.split(",");
                walls[utils.asGridCoord(Number(x), Number(y))] = true;
            })
            return walls;
        }(),
        cutsceneSpaces: {
