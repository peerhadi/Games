...
canvas.width = 1344 * dpr
canvas.height = 756 * dpr
...
function animate(backgroundCanvas) {
  ...
  for (let i = monsters.length - 1; i >= 0; i--) {
    ...
    if (player.attackBox.x + player.attackBox.width >= monster.x &&
      ...
      player.attackBox.y + player.attackBox.height >= monster.y && player.attackBox.y <= monster.y + monster.height && player.isAttacking && !player.hasHitEnemy) {
      monster.receiveHit();
...
const startRendering = async () => {
  try {
    ...
    frontRendersCanvas = await renderStaticLayers(frontRendersLayerData)
    if (!backgroundCanvas) {
